/*** ArchiveManager
 * Stores archived bookmarks in chrome.storage.local so they can be restored later.
 */
export class ArchiveManager {
  constructor(bookmarkManager) {
    this.bookmarkManager = bookmarkManager;
    this.STORAGE_KEY = 'kanbanArchiveItems';
  ...
  async removeEntries(bookmarkIds) {
    if (!Array.isArray(bookmarkIds) || bookmarkIds.length === 0) {
      return;
    }
    ...
    return new Promise((resolve) => {
      chrome.storage.local.get([this.STORAGE_KEY], (result) => {
        if (chrome.runtime.lastError) {
          console.error('Failed to load archive state:', chrome.runtime.lastError);
          resolve({ items: [] });
          return;
        }
  ...
  async _saveState(state) {
    return new Promise((resolve) => {
      chrome.storage.local.set({ [this.STORAGE_KEY]: state }, () => {
        if (chrome.runtime.lastError) {
          console.error('Failed to save archive state:', chrome.runtime.lastError);
        }
        resolve();
      });
    });
  ...
  async _folderExists(folderId) {
    return new Promise((resolve) => {
      chrome.bookmarks.get(folderId, (nodes) => {
        ...
        // Folders have no URL; treat the presence of the node as existence
